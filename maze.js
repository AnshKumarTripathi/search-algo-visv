...
// Update event listener for generateMazeButton
generateMazeButton.addEventListener("click", async () => {
  mazeSize = parseInt(gridSizeInput.value);
  await generateMazeGraph();
});

// Add an event listener to the input to update the maze when the value is changed.
...
function createMazeGrid() {
  ...
  for (let i = 0; i < mazeSize * mazeSize; i++) {
    ...
    cell.dataset.index = i; // ADD THIS LINE
...
function spawnAdditionalMazes() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    mazeWrapper.style.gridTemplateRows = `repeat(${mazeSize}, 30px)`;
    mazeWrapper.style.gridTemplateColumns = `repeat(${mazeSize}, 30px)`;
